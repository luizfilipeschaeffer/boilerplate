  ...
  const initials =
    (parts.length >= 2
      ? parts
          ...
          .join("")
      : parts[0]?.slice(0, 2).toUpperCase() ?? "") || "PA";
        ...
        <DropdownMenu>
          <DropdownMenuTrigger
            render={
              <SidebarMenuButton
                size="lg"
                tooltip={`${user.name} — ${PLATFORM_ROLE_LABELS[user.platformRole]}`}
                className="group-data-[collapsible=icon]:size-8! group-data-[collapsible=icon]:p-2! aria-expanded:bg-muted"
              />
            }
          >
            <Avatar className="size-8 rounded-lg grayscale group-data-[collapsible=icon]:size-8">
              {user.avatar ? (

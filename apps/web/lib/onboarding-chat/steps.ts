...
import { firstAccessLoginInstructions } from "@/lib/auth/login-with-code";
...
export type { ChoiceOption } from "@/lib/chat/choice-option";
  ...
  (id) => {
    ...
    return {
      ...
      next: (draft) => {
        ...
        if (id === "faseEscolhida") return "summary";

        if (id === "emiteNota") return "faseEscolhida";
...
export const ONBOARDING_STEPS: OnboardingStepDef[] = [
...

  {
    ...
    prompt: (d) =>
      ...
      firstAccessLoginInstructions(d.email.trim() || "seu e-mail").replace(
